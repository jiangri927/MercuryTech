...
class ChatViewController: MessagesViewController, MessagesDataSource {
    ...
    func setNotificationObserver(){
        NotificationCenter.default.addObserver(self, selector: #selector(didReceive(notification:)), name: .didReceiveMessage, object: nil)
    }
    
    @objc  func didReceive(notification: Notification) {
        let userInfo = notification.userInfo
        let broadcastNewID = userInfo!["messageNewID"] as! String
        
        guard let message = DataController.shared.findMessage(withId: broadcastNewID) else { return }
        guard let senderID = message.sender else { return }
    ...
    func configureNavigationBar(){
        ...
        updateTitleView(title: otherUser?.fullName ?? "", subtitle: "", baseColor: .white)
...
//        refreshControl.addTarget(self, action: #selector(loadMoreMessages), for: .valueChanged)
    ...
    func loadFirstMessages() {
        guard let sender = otherUser?.userID else { return }
        guard let receiver = AppManager.shared.currentUser?.userID else { return }
        self.messages = DataController.shared.getAllMessages(sender, receiver)
        self.messages.append(contentsOf: DataController.shared.getAllMessages(receiver, sender))
        self.messages.sort(by: {$0.time < $1.time})
        
        for message in messages {
            guard let sender = message.sender else { continue }
            guard let senderUser = DataController.shared.findUser(withId: sender) else { return }
...
extension ChatViewController: MessagesDisplayDelegate {
    ...
    func configureAvatarView(_ avatarView: AvatarView, for message: MessageType, at indexPath: IndexPath, in messagesCollectionView: MessagesCollectionView) {
        ...
        let sender = DataController.shared.findUser(withId: message.sender.senderId)
    ...
    private func insertMessages(_ data: [Any]) {
        for component in data {
            if let str = component as? String {
                ...
                DataController.shared.insertOwnMessage(withText: str, receiver: otherUserID) { msg in
                    ...
                    let dic = [
                        ...
                        MessageKeys.receiver: message.receiver!,
                        MessageKeys.received: message.received,
                        MessageKeys.broadcast: message.broadcast,
                        MessageKeys.mesh: message.mesh,
                        MessageKeys.date: message.time,
                        MessageKeys.deviceType: message.deviceType
                        ] as [String : Any]
                    BridgefyManager.shared.sendMessage(dic, otherUserID)
                }
            }
        }
    }
